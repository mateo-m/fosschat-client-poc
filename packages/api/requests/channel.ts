import type { TGetChannelMembersParams, TGetChannelMembersResponse200, TGetChannelMessagesParams, TGetChannelMessagesResponse200, TGetChannelParams, TGetChannelResponse200 } from "../types/channel";
...
export const getMessages = (params: TGetChannelMessagesParams): Promise<TGetChannelMessagesResponse200> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      const pageSize = 20;
      const totalMessages = 100; // Simulate a total of 100 messages in the channel
      const startIndex = params.cursor
        ? Math.max(0, parseInt(params.cursor, 10) + 1)
        : 0;
      const paginatedMessages: TMessage[] = Array.from({ length: pageSize }, (_, i) => ({
        ...
        reactions: i % 3 === 0 ? [{
          userIds: Array.from({ length: Math.floor(Math.random() * 10) + 1 }, (_, j) => `${(startIndex + i + j) % 50 + 1}`),
          emote: {
            guildId: params.guildId,
...
export const getMembers = (params:TGetChannelMembersParams): Promise<TGetChannelMembersResponse200> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      const pageSize = 20;
