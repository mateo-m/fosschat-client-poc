...
import type {
  TGetGuildChannelsParams,
  TGetGuildMembersParams,
  TGetGuildMembersResponse200,
  TGetGuildParams,
  TGetGuildResponse200,
  TGuild as TGuild,
} from '../types/guild';
...
const getMembers = (params: TGetGuildMembersParams): Promise<TGetGuildMembersResponse200> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      const nextCursor =
        hasMore && paginatedMembers.length
          ? (paginatedMembers[paginatedMembers.length - 1]?.id ?? null)
          : null;
      const previousCursor =
        startIndex > 0 ? (mockedGuildMembers[startIndex - 1]?.id ?? null) : null;
...
const getChannels = (params: TGetGuildChannelsParams) => {
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      const nextCursor =
        hasMore && paginatedChannels.length
